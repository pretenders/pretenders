...
def add_test_preset(
    rule="POST /fred/test/one", body=b"You tested fred well", status=200
):
...
def test_get_presets():
    ...
    response = requests.get(
        "http://localhost:8000/preset/{0}".format(http_mock.pretend_access_point_id)
    )
...
def test_setup_with_zero_or_negative_times():
    ...
    http_mock.reset()
    assert_raises(ConfigurationError, http_mock.when("POST").reply, status=202, times=0)

    assert_raises(
        ConfigurationError, http_mock.when("POST").reply, status=202, times=-2
    )
...
def test_multiple_http_mocks_independently_served():
    ...
    http_mock_1.when("GET /test_mock1_get").reply(
        b"You tested a get", 201, times=FOREVER
    )
    http_mock_2.when("GET /test_mock2_get").reply(
        b"You tested a get", 200, times=FOREVER
    )
...
def test_header_matching_with_match_headers():
    ...
    response, data = fake_client.get(
        url="/test-headers", headers={"Another-Header": "A"}
    )
...
def test_etag_workflow():
    ...
    response, data = fake_client.get(
        url="/test-etag", headers={"If-None-Match": "A12345"}
    )
...
def test_list_history():
    ...
    response = requests.get(
        "http://localhost:8000/history/{0}".format(http_mock.pretend_access_point_id)
    )
...
def test_reply_depending_on_body():
    http_mock.reset()
    http_mock.when("POST /hello", body="1.*").reply(b"First", times=FOREVER)
    http_mock.when("POST /hello", body="2.*").reply(b"Second", times=FOREVER)
...
def test_reply_depending_on_data():
    ...
    http_mock.when("POST /hello", data={"two": "second"}).reply(
        b"Second", times=FOREVER
    )
    ...
    response, data = fake_client.post(url="/hello", body="other=values")
    assert_equals(response.status, 404)
...
def test_mock_timeout_behaviour():
    ...
    http_mock.reset()
    http_mock.when("GET /test-long-process").reply(
        b"", status=200, times=FOREVER, after=20
    )
    assert_raises(socket.timeout, timeout_fake_client.get, url="/test-long-process")
