...
def get_timedelta_from_string(string):
    data = string.split(":")
    time = timedelta(hours=int(data[0]), minutes=int(data[1]), seconds=float(data[2]))
    return time
...
class PretenderModel(object):
    ...
    def __str__(self):
        return "Name: {name}, last_call: {last_call}, timeout: {timeout}".format(
            **self.__dict__
        )
    ...
    def is_expired(self):
        return (
            self.timeout != FOREVER and datetime.now() - self.last_call > self.timeout
        )
