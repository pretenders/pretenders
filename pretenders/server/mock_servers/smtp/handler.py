...
from pretenders.common.constants import RETURN_CODE_PORT_IN_USE, PRETEND_PORT_RANGE
...
class SmtpHandler(object):
    ...
    def get_or_create_pretender(self, name, timeout):
        ...
        for port_number in self.available_ports():
            LOGGER.info(
                "Attempt to start smtp pretender on port {0}, timeout {1}".format(
                    port_number, timeout
                )
            )
            ...
            if process.returncode == RETURN_CODE_PORT_IN_USE:
                LOGGER.info(
                    "Return code already set. " "Assuming failed due to socket error."
                )
                continue
            ...
            LOGGER.info(
                "Started smtp pretender on port {0}. name {1}. pid {2}".format(
                    port_number, name, process.pid
                )
            )
            return json.dumps(
                {"full_host": "localhost:{0}".format(port_number), "id": name}
            )
